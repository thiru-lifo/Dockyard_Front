...
export class ServiceSettingComponent implements OnInit {
  ...
  populateForms(users) {
    ...
    this.editForm.patchValue({service:users.service.id});
  ...
  getpackage_setting() {
      this.api.getAPI(environment.API_URL + "configuration/service_rules").subscribe({
      next: (res) => {
  ...
  crudSubmit() {
    ...
    if (this.editForm.invalid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "configuration/service_rules/details",
          this.editForm.value
        )
        .subscribe((res)=>{
  ...
  delete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "configuration/service_rules/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          if(res.status==environment.SUCCESS_CODE) {
  ...
  getservice() {
    this.api.getAPI(environment.API_URL + "master/services?status=1").subscribe({
      next: (res) => {
