...
@Component({
  selector: 'app-code-commands-catalog',
  templateUrl: './code-commands-catalog.component.html',
  styleUrls: ['./code-commands-catalog.component.scss']
})
export class CodeCommandsCatalogComponent implements OnInit {
  form:FormGroup;
  error="";
  crudName="Add";
  codeCommandCatalogList:[];
  filterValue:any;
  isReadonly=false;
  moduleAccess:any;
  ErrorMsg:any;
  ...
  public permission={
   ...
   getcode_command_catalog(){
     this.api.getAPI(environment.API_URL + "master/code_command_catalog").subscribe({
       next: (res) => {
         this.logger.log('commandCatalog',res);
         this.dataSource = new MatTableDataSource(res.data);
         this.codeCommandCatalogList = res.data;
   ...
   delete(id){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/code_command_catalog/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          if(res.status==environment.SUCCESS_CODE) {
            this.notification.warn('Coded command catalog details '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getcode_command_catalog();
          } else {
   ...
   crudSubmit(){
     if (this.form.invalid) {
       ...
       this.api
         .postAPI(
           environment.API_URL + "master/code_command_catalog/details",
           this.form.value
         )
         .subscribe((res)=>{
