...
@Component({
  selector: 'app-moh-radiology',
  templateUrl: './moh-radiology.component.html',
  styleUrls: ['./moh-radiology.component.scss']
})
export class MohRadiologyComponent implements OnInit {
 form:FormGroup;
 error="";
 crudName="Add";
 mohList:[];
 ...
 ErrorMsg:any;
  ...
  getmoh(){
    this.api.getAPI(environment.API_URL + "master/mohradiology").subscribe({
      next: (res) => {
        this.logger.log('mohRadiology',res);
        this.dataSource = new MatTableDataSource(res.data);
        this.mohList = res.data;
  ...
  update(moh){
   ...
   this.isReadonly=false;
   this.form.enable();
   this.logger.info(moh);
   this.initialize();
   this.populateForms(moh);
  ...
  delete(id){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/mohradiology/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          if(res.status==environment.SUCCESS_CODE) {
            this.notification.warn('MOH radiology details '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getmoh();
          } else {
  ...
  crudSubmit(){
    if (this.form.invalid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/mohradiology/details",
          this.form.value
        )
        .subscribe((res)=> {
