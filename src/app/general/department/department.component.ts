...
export class DepartmentComponent implements OnInit {
  ...
  getDepartment() {
    this.api
      ...
      .subscribe((res) => {
        this.logger.log('department',res);
  ...
  onView(DashboardComponent) {
    this.crudName = 'View';
    this.isReadonly=true;
    this.editForm.disable();
    this.populate(DashboardComponent);
    var element = <HTMLInputElement> document.getElementById("exampleCheck1");
    if(this.editForm.value.status == 1) {
     element.checked = true;
    }
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/department/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          if(res.status==environment.SUCCESS_CODE) {
            this.notification.warn('Department details '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getDepartment();
          } else {
  ...
  onSubmit() {
    if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/department/details",
          this.editForm.value
        )
        .subscribe((res) => {
