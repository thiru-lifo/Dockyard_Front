...
export class LookupTypeComponent implements OnInit {
  ...
  public permission={
   ...
   getlookupType(){
     this.api.getAPI(environment.API_URL + "master/lookup_type").subscribe({
       next: (res) => {
        this.logger.log('lookupType',res);
  ...
  onView(moh) {
   ...
   delete(id){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/lookup_type/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          if(res.status==environment.SUCCESS_CODE) {
            this.notification.warn('Lookup type details '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getlookupType();
          } else {
   ...
   crudSubmit(){
     if (this.form.invalid) {
       ...
       this.api
         .postAPI(
           environment.API_URL + "master/lookup_type/details",
           this.form.value
         )
         .subscribe((res)=> {
