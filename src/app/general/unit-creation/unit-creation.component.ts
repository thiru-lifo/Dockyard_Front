...
export class UnitCreationComponent implements OnInit {
  ...
  getunit(){
    this.api.getAPI(environment.API_URL + "master/unit").subscribe({
      next: (res) => {
        this.logger.log('unitCreation',res);
        this.dataSource = new MatTableDataSource(res.data);
        this.unitCreationList = res.data;
      ...
      this.api.postAPI(environment.API_URL + "master/unit/details", {
        ...
        status: 3,
          ...
          if(res.status==environment.SUCCESS_CODE) {
            this.notification.warn('Unit creation details '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getunit();
          } else {
