...
moment=moment;
gls_custom_dashboard:any;
selectedWidgets=[];

  onChangeWidget()
  {
    localStorage.setItem('gls_custom_dashboard',this.api.encryptData(this.gls_custom_dashboard));
  }
...
public projectList = [];
  ...
  ngOnInit(): void {
    ...
    this.gls_custom_dashboard=localStorage.getItem('gls_custom_dashboard')?this.api.decryptData(localStorage.getItem('gls_custom_dashboard')):this.selectedWidgets;
     this.getListing();
     this.getProjectLog;
  }
  ...
  {
    return inArray(widgetName,this.gls_custom_dashboard);
  }
...
projectlog:any;
  getProjectLog(data) {
    ...
    //console.log(param)
    this.api
      .getAPI(environment.API_URL + "transaction/projectlog?project_id="+data+"&module_id=2")
      .subscribe((res) => {
