...
@Component({
  selector: 'app-gls-document-generation',
  templateUrl: './gls-document-generation.component.html',
  styleUrls: ['./gls-document-generation.component.scss']
})
export class GlsDocumentGenerationComponent implements OnInit {
  ...
  getAuthority() {
    this.api
      ...
      .getAPI(environment.API_URL + "transaction/global_transaction_download?module_id=2&approved_status=2&status=1")
      .subscribe((res) => {
...
downloadsection(country) {
  ...
  // download(){
    ...
    window.open(environment.API_URL+'transaction/global_transaction_gls/report/'+country)

  }
  downloadpdf(country) {
  
    window.open(environment.API_URL+'transaction/global_transaction_glspdf/report/'+country)
  ...
  pdfOption(country) {
      this.editForm.patchValue({project_id: country.project.id});
        this.api.displayLoading(true)
        this.api.getAPI(environment.API_URL + 'transaction/global_transaction_get_gls_pdf/report/'+country.id).subscribe((res)=>{
          this.api.displayLoading(false)
          if(res.status==environment.SUCCESS_CODE) {
