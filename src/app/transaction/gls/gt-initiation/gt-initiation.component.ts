...
@Component({
  selector: 'app-gt-initiation.component',
  templateUrl: './gt-initiation.component.html',
  styleUrls: ['./gt-initiation.component.scss']
})
export class GTInitiationComponent implements OnInit {
  ...
  ngOnInit(): void {
  this.subModuleSelected = Number(1);
    ...
    if (this.editFormApproval.valid) {
      ...
      .subscribe((res) => {
        ...
        if(res.status==environment.SUCCESS_CODE){
          ...
          this.getListing();
          this.notification.warn('GLS Initiation notes Approval Updated Sucessfully');
