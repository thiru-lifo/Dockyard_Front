...
@Component({
  ...
  templateUrl: './gt-receipt-rfi.component.html',
  styleUrls: ['./gt-receipt-rfi.component.scss']
})
export class GTReceiptRFIComponent implements OnInit {
  ...
  populate(data) {
    ...
    this.global_psr1 = data['sub_module'][6]['section']
...
getProject(code) {
  this.api
    .getAPI(environment.API_URL + "transaction/global_transaction_edit_1?approved_status=2&sub_module_id=6")
    .subscribe((res) => {
...
console.log(this.FinalArray,'dsdd');
  ...
  getListing() {
    this.api
    .getAPI(environment.API_URL + "transaction/global_transaction_edit_1?module_id=1&sub_module_id=7")
      .subscribe((res) => {
  ...
  editOption(initiation) {
    ...
    console.log(this.editt,'aaaa')
     this.getProject("edit");
    this.crudName = "Submit";


    this.global_psr1 = initiation['sub_module'][6]['section']
    // console.log("wwww", initiation);
    // console.log("AAAA", this.global_psr1);
    console.log("id", initiation);
    ...
    if (this.editFormApproval.valid) {
      ...
      .subscribe((res) => {
        ...
        if(res.status==environment.SUCCESS_CODE){
          ...
          this.getListing();
          this.notification.warn('PSR Receipt RFI Approval Updated');
