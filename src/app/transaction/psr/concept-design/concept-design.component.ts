...
@Component({
  selector: 'app-concept-design',
  templateUrl: './concept-design.component.html',
  styleUrls: ['./concept-design.component.scss']
})
export class ConceptDesignComponent implements OnInit {
  ...
  public editForm = new FormGroup({
    ...
    refinement_of_values: new FormControl("",[Validators.required]),
    powering_requirements: new FormControl("",[Validators.required]),
    total_heat_load_calculation: new FormControl("",[Validators.required]),
  });
  ...
  populate(data) {
    ...
    this.crudForm.patchValue({project_id:data.project.id,total_heat_load_calculation:data.total_heat_load_calculation
,powering_requirements:data.powering_requirements,refinement_of_values:data.refinement_of_values});
...
getProject(code) {
  this.api
    .getAPI(environment.API_URL + "transaction/psr/inputs_for_staff_requirement_equipment/global_master")
    .subscribe((res) => {
...
console.log(this.FinalArray,'dsdd');
  ...
  {
        this.api
          .getAPI(environment.API_URL + "transaction/psr/concept_design")
          .subscribe((res) => {
            ...
            this.logger.log('concept_design',res.data)
          });

          //console.log(this.dataListPending)
  }
  ...
  saveConceptDesign()
  {
    ...
    {
      this.api.postAPI(environment.API_URL + "transaction/save_conceptDesign",this.crudForm.value).subscribe((res) => {
  ...
  onSubmitApproval() {
    if (this.editFormApproval.valid) {
      this.api
      .postAPI(
        environment.API_URL + "transaction/psr/concept_design/crud",
      ...
      .subscribe((res) => {
        ...
        if(res.status==environment.SUCCESS_CODE){
          this.notification.success(res.message);
          this.notification.warn('PSR Concept Design Approval Updated');
  ...
  onDelete(data) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/psr/concept_design/crud", {
          id: data.id,
          status: 3,
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Psr Concept Design '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getListing();
          } else {
  ...
  public editFormSendMail = new FormGroup({
    id: new FormControl(""),
    concept_design: new FormControl("", [Validators.required]),
  ...
  senEmail(design) {
    console.log(design,'WWWWWWw')
    this.editFormSendMail.patchValue({
      concept_design: design.id,
      status: "1",
    });
  ...
  onSubmitSendEmail(){
    ...
    if (this.editFormSendMail.valid) {
      ...
      formData.append('id','');
      formData.append('concept_design',this.editFormSendMail.value.concept_design);
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/psr/concept_design_send_email/crud",
          formData
        )
        .subscribe((res) => {
          this.logger.log('response',res);
  ...
  getDocumentListing(design) {

    //alert(initiation.id)
    this.api
      .getAPI(environment.API_URL + "transaction/psr/concept_design_document?status=1&concept_design="+design.id)
      .subscribe((res) => {
   ...
   onDeleteDocument(design) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/psr/concept_design_document/crud", {
          id: design.id,
          status: 3,
          document_name: design.document_name,
          document_remark: design.document_remark
        }).subscribe((res)=>{
          this.logger.log('response',res);
          if(res.status==environment.SUCCESS_CODE) {
            this.logger.info('delete')
            ...
            let design_id = this.editFormDocument.value.concept_design;
            //alert(initiation_id);
            this.getDocumentListing({id:design_id});
  ...
  document(design) {
    ...
    this.getDocumentListing(design)
  ...
  onSubmitDocument() {
    ...
    if (this.editFormDocument.valid) {
      ...
      const formData = new FormData();
      formData.append('id','');
      formData.append('concept_design', this.editFormDocument.value.concept_design);
      ...
      this.api
      .postAPI(
        environment.API_URL + "transaction/psr/concept_design_document/crud",
        formData
      )
      .subscribe((res) => {
        this.logger.log('response',res);
