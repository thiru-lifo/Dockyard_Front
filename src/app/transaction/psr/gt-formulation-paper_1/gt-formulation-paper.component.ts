...
export class GTFormulationPaperComponent implements OnInit {
  ...
  checkValue()
  {
    console.log("this.approach_paper_form",this.approach_paper_form)
...
getProject(code) {
  this.api
    ...
    //.getAPI(environment.API_URL + "transaction/approve_list")
    .subscribe((res) => {
...
for (var i = 0; i < res.data.length; ++i) {
  ...
  for (var j = 0; j < this.dataListPending.length; ++j) {
      if (this.dataListPending[j].project.id == res.data[i].project) {
          found = true;
          break;
      }
  }
...
console.log(this.FinalArray,'dsdd');
  ...
  getListing() {
    this.api
    .getAPI(environment.API_URL + "transaction/global_transaction_edit_1?module_id=1&sub_module_id=2")
      .subscribe((res) => {
  ...
  onSubmit(type='') {
    ...
    var url;
    if(type=="add"){
      this.transaction = this.approach_paper_form;
    ...
    console.log(this.transaction, url)
     ...
     console.log(this.project_id,"projectID")
