...
@Component({
  ...
  templateUrl: './gt-incorporation.component.html',
  styleUrls: ['./gt-incorporation.component.scss']
})
export class GTIncorporationComponent implements OnInit {
  ...
  checkValue()
  {
    console.log("this.incorporation",this.incorporation)
  ...
  populate(data) {
    ...
    this.global_psr1 = data['sub_module'][5]['section']
...
getProject(code) {
  this.api
    .getAPI(environment.API_URL + "transaction/global_transaction_edit_1?approved_status=2&sub_module_id=5")
    .subscribe((res) => {
...
console.log(this.FinalArray,'dsdd');
  ...
  getListing() {
    this.api
    .getAPI(environment.API_URL + "transaction/global_transaction_edit_1?module_id=1&sub_module_id=6")
      .subscribe((res) => {
...
editt:any;
  ...
  editOption(initiation) {
    ...
    this.global_psr1 = initiation['sub_module'][5]['section']
    ...
    if (this.editFormApproval.valid) {
      ...
      .subscribe((res) => {
        ...
        if(res.status==environment.SUCCESS_CODE){
          ...
          this.getListing();
          this.notification.warn('PSR Incorporation of Design Inputs Updated');
...
projecid:any;
  ...
  onSubmit(type='') {
    ...
    var url;
    if(type=="add"){
      this.transaction = this.incorporation;
