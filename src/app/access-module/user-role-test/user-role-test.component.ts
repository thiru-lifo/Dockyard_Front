...
@Component({
  selector: 'app-users',
  templateUrl: './user-role-test.component.html',
  styleUrls: ['./user-role-test.component.scss']
})
export class UserRoleTest implements OnInit {
  ...
  populate(data) {
    ...
    this.itemsTest = this.docFormTest.get('itemsTest') as FormArray;
    this.clearFormArray(this.items);
    this.clearFormArray(this.itemsTest);
  ...
  create() {
    ...
    this.itemsTest = this.itemsTest.get('itemsTest') as FormArray;
    this.clearFormArray(this.itemsTest);
    this.itemsTest.push(this.formBuilder.group({designation1:''}))
  }
