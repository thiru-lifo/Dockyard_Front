...
export class CommandComponent implements OnInit {
  ...
  getCommand() {
    this.api
      .getAPI(environment.API_URL + "master/command")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/command/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Command '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getCommand();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/command/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
