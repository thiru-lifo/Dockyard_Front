...
export class TemplateComponent implements OnInit {
  ...
  getTemplate() {
    ...
    this.api
      .getAPI(environment.API_URL + "master/template?"+this.param)
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/template/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/template/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
