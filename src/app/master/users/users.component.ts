...
export class UsersComponent implements OnInit {
  ...
  populate(data) {
    ...
    if(data.data_access.length>0)
    {
      ...
      for(let i=0;i<data.data_access.length;i++)
      {
        ...
        // module_id = data.data_access[i].module.id;
        form_id = data.data_access[i].form;
        //sub_module_id = data.data_access[i].sub_module.id;
        ...
        /*sub_module_id=[];
        for(let j=0;j< data.data_access[i].sub_module.length;j++)
        {
          let sId = data.data_access[i].sub_module[j].sub_module['id'];
          sub_module_id.push(sId);
        }*/
        ...
        this.items.push(this.formBuilder.group({form_id: form_id}));
        console.log(this.items,"KKKKKKKKKKKKkkk")
  ...
  moduleList=[];
  getModule() {
    this.api.getAPI(environment.API_URL + "master/module?status=1").subscribe((res) => {        
  ...
  subModuleList=[];
  // getSubModule() {
  //   this.api.getAPI(environment.API_URL + "master/sub_module?status=1").subscribe((res) => {        
  ...
  getSubModule() {
    let searchString='';
    // let searchString=module_id?'&module_id='+module_id:'';
    this.api
     .getAPI(environment.API_URL + "master/sub_module?&status=1"+searchString)
     .subscribe((res) => {
       this.subModuleList = res.data;
       console.log('UserListship',this.subModuleList)
     });
   }
  ...
  getforms() {
      this.api
      .getAPI(environment.API_URL + "transaction/forms-without-mapping?status=1")
      .subscribe((res)=> {
        this.forms = res.data;
