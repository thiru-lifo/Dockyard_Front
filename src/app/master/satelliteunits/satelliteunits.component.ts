...
@Component({
  selector: 'app-satelliteunits',
  templateUrl: './satelliteunits.component.html',
  styleUrls: ['./satelliteunits.component.scss']
})
export class SatelliteunitsComponent implements OnInit {
  ...
  populate(data) {
    ...
    setTimeout(()=>{
      this.editForm.patchValue({command:data.command.id});
...
getCommand(trial_unit_id= '') {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/satellite_units/crud", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Satellite unit '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getSatelliteUnits();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/satellite_units/crud",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
