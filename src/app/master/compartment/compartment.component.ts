...
sectiong:any;
  getgSection() {
    this.api
      .getAPI(environment.API_URL + "master/global_section?status=1&order_type=asc&type_compartment=1")
      .subscribe((res) => {
  ...
  getSubSections(global_section_id='') {
    ...
    let filter='?status=1&order_type=asc&type_compartment=1';
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/compartment/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Compartment '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getCompartment();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/compartment/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
