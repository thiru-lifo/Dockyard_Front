...
@Component({
  ...
  templateUrl: './project_type.component.html',
  styleUrls: ['./project_type.component.scss']
})
export class ProjectTypeComponent implements OnInit {
  ...
  getProjectType() {
    this.api
      .getAPI(environment.API_URL + "master/project_type")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/project_type/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Project type '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProjectType();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/project_type/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
