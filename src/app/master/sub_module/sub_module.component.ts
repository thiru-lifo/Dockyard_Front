...
@Component({
  ...
  templateUrl: './sub_module.component.html',
  styleUrls: ['./sub_module.component.scss']
})
export class SubModuleComponent implements OnInit {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/sub_module/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/sub_module/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
