...
export class PSRSectionComponent implements OnInit {
  ...
  populate(data) {
    ...
    if(data.units)
    {
      let selectedUnits=arrayColumn(data.units,'unit');
      selectedUnits=arrayColumn(selectedUnits,'id');
      console.log('selectedUnits',selectedUnits);
      this.editForm.patchValue({unit_id:selectedUnits});
    }
  ...
  getProject() {
    this.api
      .getAPI(environment.API_URL + "transaction/psr/section")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/psr/section/details", {
          id: id,
          status: 3,
          unit_id:'',
        }).subscribe((res)=>{
  ...
  onSubmit() {
    ...
    //return false;
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/psr/section/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
