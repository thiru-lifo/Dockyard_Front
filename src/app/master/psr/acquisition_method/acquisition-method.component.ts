...
@Component({
  selector: 'app-ships',
  templateUrl: './acquisition-method.component.html',
  styleUrls: ['./acquisition-method.component.scss']
})
export class AcquisitionMethodComponent implements OnInit {
  ...
  getProject() {
    this.api
      .getAPI(environment.API_URL + "transaction/psr/acquisition_method")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/psr/acquisition_method/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Acquistion Method '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProject();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/psr/acquisition_method/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
