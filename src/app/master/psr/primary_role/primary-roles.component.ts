...
@Component({
  selector: 'app-ships',
  templateUrl: './primary-roles.component.html',
  styleUrls: ['./primary-roles.component.scss']
})
export class PrimaryRolesComponent implements OnInit {
  ...
  getProject() {
    this.api
      .getAPI(environment.API_URL + "transaction/psr/primary_roles")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/psr/primary_roles/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Primary Role '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProject();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/psr/primary_roles/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
