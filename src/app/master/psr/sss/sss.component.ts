...
export class SSSComponent implements OnInit {
  ...
  getProject() {
    this.api
      .getAPI(environment.API_URL + "transaction/psr/sss")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/psr/sss/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('PSR SSS '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProject();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/psr/sss/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
