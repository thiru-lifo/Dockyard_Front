...
@Component({
  selector: 'app-ships',
  templateUrl: './sub_section.component.html',
  styleUrls: ['./sub_section.component.scss']
})
export class PSRSubSectionComponent implements OnInit {
  ...
  getProject() {
    this.api
      .getAPI(environment.API_URL + "transaction/psr/sub_section")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/psr/sub_section/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Sub-Section '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProject();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/psr/sub_section/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
