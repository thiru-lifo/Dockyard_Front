...
@Component({
  ...
  templateUrl: './unit_type.component.html',
  styleUrls: ['./unit_type.component.scss']
})
export class UnitTypeComponent implements OnInit {
  ...
  getUnitType() {
    this.api
      .getAPI(environment.API_URL + "master/unit_type")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/unit_type/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/unit_type/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
