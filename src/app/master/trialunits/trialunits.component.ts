...
@Component({
  selector: 'app-trialunits',
  templateUrl: './trialunits.component.html',
  styleUrls: ['./trialunits.component.scss']
})
export class TrialunitsComponent implements OnInit {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/trial_units/crud", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Trial unit '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getTrialUnits();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/trial_units/crud",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
