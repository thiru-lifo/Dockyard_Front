...
getShips(trial_unit_id='',satellite_unit_id='') {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/equipments/crud", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Equipments '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getEquipments();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/equipments/crud",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
  ...
  {
    this.api.displayLoading(true);
    this.api.postAPI(environment.API_URL + "transaction/sync/equipments", {}).subscribe((res)=>{
        this.logger.log('response',res);
        this.api.displayLoading(false);
        if(res.status==environment.SUCCESS_CODE) {
          this.notification.success(res.message);
