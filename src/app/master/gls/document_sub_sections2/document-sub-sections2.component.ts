...
@Component({
  selector: 'app-ships',
  templateUrl: './document-sub-sections2.component.html',
  styleUrls: ['./document-sub-sections2.component.scss']
})
export class DocumentSubSections2Component implements OnInit {

  displayedColumns: string[] = [
    "document_sections",
    "document_sub_sections",
    "name",
    "code",
    "status",
    "view",
  ...
  populate(data) {
    ...
    this.editForm.patchValue({document_sections:data.document_sections.id});
    this.getDocumentSubSections(data.document_sections.id);
    setTimeout(()=>{
      this.editForm.patchValue({document_sub_sections:data.document_sub_sections?data.document_sub_sections.id:''});
    },500);
  ...
  //   this.api
  //     .getAPI(environment.API_URL + "transaction/gls/document_sub_sections?status=1")
  //     .subscribe((res) => {
  //       this.documentSubSections = res.data;
  //       //console.log('#####',this.documentSubSections)
  //     });
  // }
  ...
  getProject() { 
    this.api
      .getAPI(environment.API_URL + "transaction/gls/document_sub_sections2")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/gls/document_sub_sections2/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Document sub sections2 '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProject();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/gls/document_sub_sections2/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
