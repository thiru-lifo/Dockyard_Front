import { Component, OnInit, ViewChild, Input, ElementRef, OnChanges, SimpleChanges, DoCheck } from "@angular/core";
...
@Component({
  selector: 'app-ships',
  templateUrl: './document-annexures.component.html',
  styleUrls: ['./document-annexures.component.scss']
})
export class AnnexuresComponent implements OnInit {
  ...
  getProject() { 
    this.api 
      .getAPI(environment.API_URL + "transaction/gls/annexures")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/gls/annexures/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Annexures '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProject();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/gls/annexures/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
