...
@Component({
  selector: 'app-ships',
  templateUrl: './document-sub-sections.component.html',
  styleUrls: ['./document-sub-sections.component.scss']
})
export class DocumentSubSectionsComponent implements OnInit {
  ...
  getProject() {
    this.api
      .getAPI(environment.API_URL + "transaction/gls/document_sub_sections")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "transaction/gls/document_sub_sections/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Document sub sections '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProject();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/gls/document_sub_sections/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
