...
@Component({
  selector: 'app-trialtypes',
  templateUrl: './trialtypes.component.html',
  styleUrls: ['./trialtypes.component.scss']
})
export class TrialtypesComponent implements OnInit {
  ...
  getTrialTypes() {
    this.api
      .getAPI(environment.API_URL + "master/trial_types")
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/trial_types/crud", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/trial_types/crud",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
