...
@Component({
  ...
  templateUrl: './global_section.component.html',
  styleUrls: ['./global_section.component.scss']
})
export class GlobalSectionComponent implements OnInit {
  ...
  populate(data) {
    ...
    this.editForm.patchValue({Class:data.Class.id});
    ...
    //this.editForm.setValue({module:data.module.id,sub_module:data.sub_module.id})
    console.log('111',data.sub_module.id)
setTimeout(()=>{
      this.editForm.patchValue({sub_module:data.sub_module.id});
    },500);
...
module:any;
  ...
  getSection() {
    ...
    this.api
      .getAPI(environment.API_URL + "master/global_section?"+this.param)
      .subscribe((res) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/global_section/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Global Section '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getSection();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/global_section/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
...
classes:any;
  ...
  // param:any;
  search(){
  let type=this.searchForm.value.classe?"Class="+this.searchForm.value.classe:"";
  type+=this.searchForm.value.module?"&module="+this.searchForm.value.module:"";
