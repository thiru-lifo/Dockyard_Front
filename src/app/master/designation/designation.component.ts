...
export class DesignationComponent implements OnInit {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/designation/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Designation '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getProjectType();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/designation/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
