...
export class SectionsComponent implements OnInit {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/sections/crud", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          this.logger.log('response',res);
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Sections '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getSections();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/sections/crud",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
